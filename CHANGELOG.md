...
## [Unreleased]
### Added
- Генерация ответов с цитатами по чанкам, группировка подсветки по страницам с OCR и корректировка
  padding прямоугольников.
- В таблице `assets` появилось поле `postcard_score` — числовая оценка «открыточности» кадра от 1 (совсем не открытка) до 10 (почти идеальная открытка), которое проставляет vision-модель и которое теперь доступно в отборах и админке.
- Ассеты с `postcard_score ≥ 6` автоматически получают тег `postcard`, а в подписи ассет-канала для таких кадров появляется строка «Открыточность: N/10», чтобы операторы сразу видели сильные открытки.
...
- Новая рубрика «Новогоднее настроение» (`new_year`): расписание собирает 1–4 кадра с тегом `new_year`, отсортированные по
  `postcard_score`, избегает повторов в продакшене, а ассеты с новогодним оформлением автоматически получают этот тег при
  загрузке.
