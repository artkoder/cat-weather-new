...
- Для ручного запуска откройте карточку рубрики и нажмите `▶️ Запустить` (боевой слот) или `🧪 Тест` (тестовый слот). Теперь бот не публикует пост сразу: в операторском чате появляется превью дропа с кнопками для перегенерации фотографий, перегенерации подписи, добавления дополнительных инструкций и финальной отправки в боевой или тестовый канал.
