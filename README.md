...
## Подбор фото: правила и стадии

Алгоритм «морской» рубрики использует многоступенчатый подбор кадров и учитывает
три ключевых сигнала: видимость неба (`sky_visible`), классификацию неба
(`photo_sky`) и оценку волнения (`photo_wave`). Для повышения качества база
хранит метаданные `photo_doy` (день года из EXIF/времени съёмки), `photo_wave`
и трёхзначный `sky_visible` (`true`, `false`, `unknown`).

- **B0 (строгий)** — допускает только кадры с видимым небом, чьи теги попадают в
  разрешённый набор (например, `sunny`/`mostly_clear` для ясной погоды). Волновой
  коридор ±1.0: отклонения больше отбрасываются. Сезонное окно обязательно.
- **B1 (слегка мягче)** — разрешает `sky_visible=unknown` со штрафом и расширяет
  волновой допуск до ±1.8. Небо вне ожидаемого диапазона остаётся в пуле, но
  получает ощутимый штраф.
- **B2 (мягко)** — допускает кандидатов вне сезонного окна с дополнительным
  штрафом и увеличивает коридор до ±2.5. Волны могут быть неизвестны: за это
  начисляется фиксированный штраф.
- **AN (аварийный режим)** — включается, если предыдущие стадии пусты. Даже при
  штиле кадры с волной >4 не пройдут, а за точное совпадение по небу выдается
  бонус. Отсутствие даты съёмки дополнительно штрафуется, чтобы при прочих равных
  побеждали кадры с заполненным `photo_doy`.

Во всех стадиях финальный балл складывается из возрастного бонуса (редкие кадры
выше), штрафа за волну за пределами коридора, штрафов/бонусов за небо и
возможного сезонного штрафа. Логи содержат покомпонентный разбор (`score_components`),
использованный коридор и актуальный таргет волнения, что позволяет объяснить
итоговый выбор.

Для исправления исторических данных используйте CLI `weather_migration.py`:

```bash
python weather_migration.py --database path/to/bot.db --dry-run \
    --fill-doy --recalc-sky-visible --backfill-wave
```

Команда выводит отчёты по количеству обновлённых строк; флаг `--dry-run`
позволяет проверить результат без записи.


## API Contract

The backend consumes the public API contract via the `api/contract` git submodule pinned to the release tag `v1.4.1`; see the official docs at https://artkoder.github.io/kotopogoda-api-contract/. The canonical OpenAPI document lives at `api/contract/openapi/openapi.yaml`; keep the repository free from alternative copies so the contract stays single-sourced.
...
- Unit tests run by default with `make test` or `pytest`
- Integration tests (marked with `@pytest.mark.integration`) require additional setup and are skipped by default
- E2E tests (marked with `@pytest.mark.e2e`) require external services and are skipped by default

To run all tests: `make test-all` or `pytest -m ""`

### Environment variables for testing

Basic tests work without secrets using safe defaults:
- `TELEGRAM_BOT_TOKEN=dummy` - Telegram API calls are mocked
- `DB_PATH=/tmp/test_bot.db` - Use temporary database for tests
- `WEBHOOK_URL=https://example.com` - Placeholder webhook URL

For integration tests, provide real credentials via environment variables.
...
- **Новогодние теги**. Ассеты с новогодним оформлением (гирлянды, ёлки, декор) получают авто‑тег `new_year` прямо при загрузке и разборе vision-пайплайном. В ассет-канале видно проставленный тег, операторы могут дотянуть его вручную для любых подходящих кадров, чтобы собрать пул под праздничную рубрику.
...
- **Rubric automation**. Ежедневные рубрики `flowers`, `guess_arch` и сезонная `new_year` поддерживаются из коробки: `flowers` builds its palette strictly from vision outputs, threads daypart weather context into the greeting, preserves per-photo descriptions, injects weather as numeric metrics plus plain-language conditions (no raw provider codes) and spells out the latest `gpt-4o` prompting rules directly in the request payload; `guess_arch` prepares a numbered architecture quiz with optional overlays and weather context. Все рубрики потребляют распознанные ассеты, очищают их после публикации, автоинициализируются при первом запуске и целиком управляются через inline-дэшборд `/rubrics`.
...
### Exporting postcard assets

Use the diagnostic exporter when you need a dump of every asset together with its postcard score, regardless of the value or whether the score lives in the `assets` column or inside `vision_results.result_json`. Run the helper with the production database path (for example `/postcard_photos_db`) and point it at the desired output file:

```bash
python scripts/export_high_score_postcards.py --db-path /postcard_photos_db --output postcard_photos_dump.csv
```

The script saves a CSV file (defaults to `postcard_photos_dump.csv` in the current directory), pretty-prints JSON blobs when `--pretty-json` is supplied and prints both the absolute output path and the number of exported rows so it is easy to share the results with the team. Superadmins can request the same export directly in Telegram via `/postcard_photos_db` (append `pretty` to format JSON columns in multiple lines); the bot sends the generated CSV back without applying any filters at all.
...
#### Открыточный вид (`postcard`)
- Публикует одну фото-открытку, которую оценивает vision-модель: селектор берёт все кадры с максимальным `postcard_score`, в первую очередь рассматривает свежие (до трёх суток) и сезонные (±30 дней) съёмки и игнорирует кадры, которые уже выходили в рубриках за последние семь дней.
- Подпись собирается через `caption_gen`: текст строго начинается с «Порадую вас красивым видом …», обязательно упоминает локацию (город или регион) из payload и заканчивается фирменной ссылкой «Полюбить 39» вместе с набором из 3–5 хэштегов.
- Для финального кадра бот автоматически накладывает полупрозрачный водяной знак LoveKaliningrad, центрируя его по горизонтали и прижимая к нижнему краю. Файл `assets/watermarks/LoveKaliningrad.png` не хранится в репозитории по лицензионным причинам — положите его вручную перед запуском. Рекомендуется использовать RGBA‑PNG шириной до 80 % кадра (≈1500 px для вертикальных снимков) и общей непрозрачностью 40–60 %, чтобы знак оставался заметным, но не перекрывал сюжет. Тесты CI упадут, если файла нет на месте.
- В отчёте «Остатки» (кнопка `Остатки` в карточке рубрики) доступна кнопка «Отправить сейчас» — она ставит внеочередную публикацию в рабочий канал через тот же пайплайн, что и расписание.
- По умолчанию рубрика выключена. Настройка каналов (боевой/тестовый), расписаний и ручных запусков выполняется из карточки `postcard` в `/rubrics`, там же задаются окна публикаций и стоп-слова для капшенов.

#### Новогоднее настроение (`new_year`)
- Работает в праздничный сезон и подхватывает ассеты с тегом `new_year`. Селектор забирает 1–4 кадра из пула без повторов в продакшене, сортирует их по убыванию `postcard_score` и собирает карусель с самыми «открыточными» снимками.
- Расписание поддерживает выбор конкретных месяцев (ноябрь–январь) и слотов, как и другие рубрики: добавьте окно публикаций через «Добавить расписание» в карточке `new_year`, а в описании слота видно выбранный месяц и канал.
- Ручной запуск доступен из карточки: `▶️ Запустить` ставит боевую публикацию, `🧪 Тест` отправляет превью в тестовый канал. Предпросмотр дропа приходит в операторский чат с кнопками для финальной отправки или перегенерации подписи.
- В отчёте «Остатки» отображается текущий запас помеченных ассетов и кнопка «Отправить сейчас», которая публикует ближайший праздничный дроп вне очереди тем же пайплайном, что и расписание.
