...
async def generate_postcard_caption(
    ...
    system_prompt_lines = [
        ...
        "Ты редактор: проверь орфографию, логику и соответствие сезона/времени съёмки. photo_year_label, photo_season и season_line — подсказки для тебя, лишнюю служебную строку система добавит сама.",
        "Ссылку «Полюбить 39» и карты добавит система — не упоминай их.",
    ]
    if season_line:
        system_prompt_lines.append(
            "Если в JSON есть season_line (например, «лето 2023»), значит дата съёмки старше 60 дней. Рассказывай о сцене строго в прошедшем времени, допускаются подводки «Вспомним…», «Напомню…», избегай слов «сейчас», «сегодня» и их форм."
        )
    elif is_out_of_season:
        system_prompt_lines.append(
            "Фото из другого сезона — говори в прошедшем времени, можно начать второе предложение с «Вспомним…», «Напомню…», избегай слов «сейчас», «сегодня» и их форм."
        )
    else:
        system_prompt_lines.append(
            "Если снимок свежий и совпадает с текущим сезоном, опиши сцену в настоящем времени, будто она происходит прямо сейчас."
        )
    if banned_words:
