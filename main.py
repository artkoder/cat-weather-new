    ...
    async def _generate_sea_caption(
        ...
        *,
        ...
        system_prompt = (
            ...
            "# Форма и тон\n"
            "Пиши ровно 2 абзаца.\n\n"
            "Вступление — 1–2 коротких предложения, тёплый спокойный тон. Можно очень коротко, вплоть до «Порадую вас морем». "
            "Разрешён ровно один уместный эмодзи только во вступлении (любой, по смыслу).\n\n"
            "Интересный факт — отдельный абзац, начинается чёткой подводкой (см. LEADS) и далее — одно компактное предложение факта.\n\n"
            f"{day_part_instruction}"
            "Не превращай вступление в «сводку погоды»: не перечисляй параметры и цифры про текущую погоду.\n\n"
            "# Лексика\n"
            "Избегай нейросетевых клише и олицетворений (в любых формах): дышит, шепчет, манит, ласкает, нежится, обнимает, "
            "зовёт/зовет, очаровывает/зачаровывает, завораживает, волшебный, по-настоящему, буквально, наполнен, вдохновляет, "
            "приглашает окунуться, голос моря, игра волн, умиротворение.\n\n"
            "Эмоцию передавай через 1–2 конкретные чувственные детали, без длинных цепочек эпитетов.\n\n"
            "# LEADS (подводки к факту)\n"
            "Используй одну из: «А вы знали?», «Интересный факт:», «Это интересно:», «Кстати:», «К слову о Балтике,», "
            "«Поделюсь фактом:», «Любопытная деталь:», «Небольшое пояснение:», «Теперь вы будете знать:», «К слову о море,».\n\n"
            "Допустима естественная эквивалентная подводка, если из неё однозначно ясно, что дальше — полезная/интересная информация (факт). "
            "Пунктуацию ставь по правилам русского языка.\n\n"
            "# Работа с fact_sentence\n"
            "Факт приходит с бэкенда. Можно мягко перефразировать, не искажая смысла и фактов "
            "(числа/названия/термины — корректные). Дай одно цельное предложение.\n\n"
            "# Ограничения по длине\n"
            "Вступление ≤ 220 символов.\n\n"
            "Весь caption ≤ 350 символов; если fact_sentence длинный, допустимо до 400.\n\n"
            "Хэштеги не включай в текст (возвращаются отдельным массивом).\n\n"
            "# Читабельность и проверка\n"
            "Текст должен читаться легко и естественно; одна мысль — одно предложение; без канцелярита.\n\n"
            "Проверь орфографию и пунктуацию перед ответом.\n\n"
