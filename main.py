...
from jobs import publish_rubric as publish_rubric_jobs
from jobs import vision as vision_jobs
...
class Bot:
    def __init__(self, token: str, db_path: str):
        ...
        self._asset_vision_schema = ASSET_VISION_V1_SCHEMA
        self._framing_allowed_values = FRAMING_ALLOWED_VALUES
        self._framing_alias_map = FRAMING_ALIAS_MAP
        self._season_translations = SEASON_TRANSLATIONS
        self._weather_tag_translations = WEATHER_TAG_TRANSLATIONS
        self._assets_debug_exif = ASSETS_DEBUG_EXIF
    ...
    async def _download_ocr_payload(
        ...
        *,
        ...
        forward_targets: list[tuple[int | str, str]] = [(target_chat_id, "primary")]
        if fallback_chat_id and str(fallback_chat_id) != str(target_chat_id):
            forward_targets.append((fallback_chat_id, "fallback"))

        last_error: str | None = None
        try:
            for forward_chat_id, attempt_label in forward_targets:
                try:
                ...
                if isinstance(copied_message_id, int):
                    cleanup_message_ids.append((forward_chat_id, copied_message_id))
            ...
            return OcrDownloadResult(
                payload=None,
                raw_bytes=None,
                error=last_error or "forward_failed",
                stage=last_error or "forward_failed",
    ...
    async def _job_vision_locked(self, job: Job) -> None:
        await vision_jobs.run_vision(self, job)
    ...
    async def publish_rubric(
        ...
        *,
        ...
        return await publish_rubric_jobs.publish_rubric(
            self,
            code,
            channel_id=channel_id,
            test=test,
