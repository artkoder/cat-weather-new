    ...
    async def _publish_sea(
        ...
        *,
        ...
        candidates = self.data.fetch_sea_candidates(
            ...
            target_wave_score=target_wave_score,
        )
        ...
        def evaluate_stage_candidate(
            ...
            if photo_wave_val is None and stage_cfg.allow_missing_wave:
                null_wave_penalty = 0.8
                components["WaveNullPenalty"] = null_wave_penalty
                score -= null_wave_penalty
        ...
        for stage_cfg in stage_sequence:
            ...
            for idx, (score_value, reason_payload, candidate_payload) in enumerate(
                ...
                penalties_breakdown = {
                    k: v for k, v in component_payload.items() if "Penalty" in k and v != 0.0
                }
                total_penalties = sum(penalties_breakdown.values())
                ...
                sea_log(
                    ...
                    sky_photo=_sky_token(candidate_payload.get("photo_sky_struct")),
    ...
    async def _generate_sea_caption(
        ...
        *,
        ...
        system_prompt = (
            ...
            "Если дан FACT — сделай короткую подводку («Знаете ли вы…», «Интересный факт:», «К слову о Балтике…», «Поделюсь фактом…») и одно предложение с фактом; избегай научного жаргона и сохрани все цифры и единицы из FACT строго в исходном виде; общий блок до 350 символов. "
