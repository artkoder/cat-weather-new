...
async def test_attach_device_emits_mobile_log(tmp_path, caplog):
    ...
    attach_event = next(record for record in caplog.records if record.message == "MOBILE_ATTACH_OK")
...
async def test_attach_device_invalid_code(tmp_path):
    ...
    async with TestServer(app) as server:
        async with TestClient(server) as client:
            ...
            assert (
                payload["message"] == "Недопустимый формат токена: ожидаем 6–8 символов A-Z и 2-9."
            )
