...
async def test_download_ocr_payload_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(bot, "_collect_asset_metadata", lambda payload: {"file_meta": {"file_id": "file-1"}})
...
async def test_download_ocr_payload_fallback_copy(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(bot, "api_request", fake_api_request)
    monkeypatch.setattr(bot, "_collect_asset_metadata", lambda payload: {"file_meta": {"file_id": "file-2"}})
    monkeypatch.setattr(bot, "_download_file", fake_download_file)
    ...
    assert result.error is None
    assert result.stage == "success"
    assert any(call[1].get("chat_id") == bot._raw_answer_scans_channel_id for call in calls if call[0] == "forwardMessage")
    await bot.close()
...
async def test_download_ocr_payload_parse_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(bot, "api_request", fake_api_request)
    monkeypatch.setattr(bot, "_collect_asset_metadata", lambda payload: {"file_meta": {"file_id": "file-3"}})
    monkeypatch.setattr(bot, "_download_file", fake_download_file)
